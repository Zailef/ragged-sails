...
func _on_entity_tree_exiting() -> void:
	...
	for instance in active_effects:
		if instance.source and is_instance_valid(instance.source) and instance.source.has_method("_on_affected_enemy_freed"):
			instance.source._on_affected_enemy_freed(entity)
